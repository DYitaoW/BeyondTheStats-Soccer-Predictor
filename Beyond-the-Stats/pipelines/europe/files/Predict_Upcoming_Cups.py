...
# PROJECT_DIR kept from shared.paths bootstrap
...
#needs to be changed in the future for predictions using sklearn before next season start!!!

# BASE_DIR set by shared.paths bootstrap above
RAW_DATA_DIR = os.path.join(BASE_DIR, "Data", "Raw_Data")

PREDICTIONS_FILE = os.path.join(PREDICTIONS_DIR, "upcoming_cup_predictions.csv") # NEW OUTPUT FILE
PRESEASON_DIR = os.path.join(os.path.dirname(__file__), "preseason")
PRESEASON_CUP_FIXTURES_FILE = os.path.join(PRESEASON_DIR, "2026_27_cup_fixture_fallback.json")
PRESEASON_LEAGUE_FALLBACK_FILE = os.path.join(PRESEASON_DIR, "2026_27_league_team_fallback.json")
...
# ESPN scoreboard API template for each competition
...
DEFAULT_CUP_FIXTURE_WINDOW_DAYS = 180
LEAGUES_CUP_COMPETITION = "North America/Leagues Cup"

# Map cup names to TheSportsDB league IDs (these IDs must be correct for each cup)
THESPORTSDB_LEAGUE_IDS = {
    "England/FA Cup": "4482",           # English FA Cup 4482
    "England/League Cup": "4570",     # English League Cup (EFL Cup) 4570
    "Europe/Champions League": "4480",             # UEFA Champions League 4480
    "Europe/Europa League": "4481",             # UEFA Europa League  4481
    "Europe/Conference League": "5071",            # UEFA Europa Conference League 5071
    "Italy/Coppa Italia": "4506",
    "Spain/Copa del Rey": "4483",
    "Germany/DFB-Pokal": "4485",
    "France/Coupe de France": "4484",
}

# --- TheSportsDB fixtures fetcher ---
def load_upcoming_matchweek_fixtures_from_thesportsdb(window_days, competition_name):
    league_id = THESPORTSDB_LEAGUE_IDS.get(competition_name)
    if not league_id:
        print(f"TheSportsDB: No league ID for {competition_name}")
        return pd.DataFrame()
    ...
    try:
        ...
        print(f"TheSportsDB API error for {competition_name}: {e}")
        return pd.DataFrame()
    ...
    rows = []
    for event in events:
        # Parse date and filter by window
        ...
        if not home_team or not away_team:
            continue
        match_time = event.get("strTime") or ""
        rows.append({
            "match_date": match_date,
            "match_datetime_et": match_time,
    ...
    if fixtures.empty:
        ...
        return fixtures
    print(f"TheSportsDB: Loaded {len(fixtures)} fixtures for {competition_name}")
    return fixtures

# Define all target cups and their identifiers/names
CUP_COMPETITIONS = {
    "FA Cup": {
        "name": "England/FA Cup",
        "espn_id": "eng.fa",  # ESPN FA Cup
        "priority": 1,
        "api_source": "espn",
    ...
    "Carabao Cup": {
        ...
        "espn_id": "eng.league_cup",  # ESPN Carabao Cup (EFL Cup)
        "priority": 2,
        "api_source": "espn",
        "fixture_window_days": 180,
    },
    "UCL": {
        ...
        "espn_id": "uefa.champions",  # ESPN UCL
        "priority": 3,
        "api_source": "espn",
        "fixture_window_days": 180,
    },
    ...
    "UECL": {
        ...
        "espn_id": "uefa.europa.conf",  # ESPN UECL
        "priority": 5,
        "api_source": "espn",
        "fixture_window_days": 180,
    },
    ...
    "Coupe de France": {
        ...
        "espn_id": "fra.coupe_de_france",
        "priority": 9,
        "api_source": "espn",
        "fixture_window_days": 180,
    },
    "Leagues Cup": {
        "name": LEAGUES_CUP_COMPETITION,
        "espn_id": "concacaf.leagues.cup",
        "priority": 10,
        "api_source": "espn",
        "fixture_window_days": 180,
    },
...
CROSS_LEAGUE_CUP_COMPETITIONS = {
    LEAGUES_CUP_COMPETITION,
}

LEAGUES_CUP_SOURCE_COMPETITIONS = (
    "United States/MLS",
    "Mexico/Liga MX",
)

# Directory holding the real per-team MLS / Liga MX statistics used to give
# Leagues Cup clubs distinct (non-fallback) predictions.  The main
...
CROSS_LEAGUE_LEAGUE_STRENGTHS = {
    "United States/MLS": 0.85,
    "Mexico/Liga MX": 0.80,
}

# --- CORE CLASSES AND UTILITIES (Copied from original script) ---
...
def merge_mls_team_data(context):
    """Merge real MLS / Liga MX per-team stats into the cup prediction context.
    ...
    """
    overall = pm.load_json_if_exists(os.path.join(MLS_TEAM_DATA_DIR, "overall_teams.json"))
    season = pm.load_json_if_exists(os.path.join(MLS_TEAM_DATA_DIR, "season_teams.json"))
    h2h = pm.load_json_if_exists(os.path.join(MLS_TEAM_DATA_DIR, "head_to_head.json"))
    form = pm.load_json_if_exists(os.path.join(MLS_TEAM_DATA_DIR, "current_form.json"))
    ls = pm.load_json_if_exists(os.path.join(MLS_TEAM_DATA_DIR, "league_strength.json"))
    squad = pm.load_json_if_exists(os.path.join(MLS_TEAM_DATA_DIR, "mls_squad_values.json"))

    if not isinstance(overall, dict) or not isinstance(season, dict):
        return 0

    overall = pm.replace_nan_with_sentinel(overall)
    season = pm.replace_nan_with_sentinel(season)
    h2h = pm.replace_nan_with_sentinel(h2h) if isinstance(h2h, dict) else {}
    form = pm.replace_nan_with_sentinel(form) if isinstance(form, dict) else {"teams": {}}
    ls = pm.replace_nan_with_sentinel(ls) if isinstance(ls, dict) else {}
    squad = squad if isinstance(squad, dict) else {}

    ctx_overall = context.setdefault("overall_teams", {})
    ctx_season = context.setdefault("season_teams", {})
    ctx_h2h = context.setdefault("head_to_head", {})
    ctx_form = context.setdefault("current_form", {})
    if not isinstance(ctx_form, dict) or "teams" not in ctx_form or not isinstance(ctx_form["teams"], dict):
        ctx_form["teams"] = {}
    ctx_ls = context.setdefault("league_strength", {})
    ctx_squad = context.setdefault("_mls_squad_values", {})

    ctx_overall.update(overall)
    for season_key, teams in season.items():
        bucket = ctx_season.setdefault(season_key, {})
        bucket.update(teams)
    for team_a, opponents in h2h.items():
        if not isinstance(opponents, dict):
            continue
        ctx_h2h.setdefault(team_a, {}).update(opponents)
    for team_name, stats in (form.get("teams", {}) or {}).items():
        ctx_form["teams"].setdefault(team_name, stats)
    for comp, strength in ls.items():
        ctx_ls.setdefault(comp, strength)
    for comp, strength in CROSS_LEAGUE_LEAGUE_STRENGTHS.items():
        ctx_ls.setdefault(comp, strength)
    for team_name, info in (squad.get("teams", {}) or {}).items():
        if isinstance(info, dict) and isinstance(info.get("squad_value_eur_m"), (int, float)):
            ctx_squad.setdefault(team_name, float(info["squad_value_eur_m"]))

    # Map each merged team to its source league so league-strength lookups in
    # predict_fixture / build_features use MLS or Liga MX rather than the cup.
    team_comp_map = context.setdefault("team_competition_map", {})
    available = context.setdefault("available_teams", [])
    available_set = set(available)
    for season_key, teams in season.items():
        source = os.path.dirname(str(season_key)).replace("\\", "/") or ""
        if source in LEAGUES_CUP_SOURCE_COMPETITIONS:
            for team_name in teams:
                team_comp_map.setdefault(team_name, source)
                if team_name not in available_set:
                    available_set.add(team_name)
                    available.append(team_name)
    return len(overall)


def build_prediction_context():
    # This function loads the ML model and all necessary historical stats.
    # It assumes pm.load_training_matches and pm.MODEL_CACHE exist and are accessible.
    print("Building prediction context: Loading ML models and historical stats...")
    ...
    apply_preseason_league_fallback_to_context(ctx)
    merged_mls = merge_mls_team_data(ctx)
    if merged_mls:
        print(f"[mls-merge] Loaded real per-team stats for {merged_mls} MLS/Liga MX teams")
    return ctx
...
def predict_fixture(row, context):
    ...
    # Override with real league when UEFA data is available
    real_home = _uefa_leagues.get(home_team)
    real_away = _uefa_leagues.get(away_team)
    if real_home:
        home_comp = real_home
    if real_away:
        away_comp = real_away
    ...
    # --- Inline probability adjustment logic from Predict_Match.py ---
    ...
    # No home_advantage or market_value shift for cups (skip those steps)
    ...
    # European cups use EU_RANDOMIZER_MAX_DELTA, domestic cups use a lower value, unknowns use a fallback
    CUP_RANDOMIZER_DELTAS = {
        "Europe/Champions League": pm.EU_RANDOMIZER_MAX_DELTA,
        "Europe/Europa League": pm.EU_RANDOMIZER_MAX_DELTA,
        "Europe/Conference League": pm.EU_RANDOMIZER_MAX_DELTA,
        "England/FA Cup": 0.045,  # Domestic cup, lower randomness
        "England/League Cup": 0.045,
        "Italy/Coppa Italia": 0.045,
        "Spain/Copa del Rey": 0.045,
        "Germany/DFB-Pokal": 0.045,
        "France/Coupe de France": 0.045,
        "North America/Leagues Cup": 0.045,
    }
    # If not mapped, use 0.035 for unknown cup competitions
    randomizer_delta = CUP_RANDOMIZER_DELTAS.get(competition, 0.035)
    ...
    return {
        ...
        "home_team": home_team,
        "away_team": away_team,
        "display_home_team": str(row.get("home_team", "")).strip(),
        "display_away_team": str(row.get("away_team", "")).strip(),
        "schedule_only": "0",
        ...
        "probability_reasoning": pm.build_reasoning_string(
            ...
            float(pred_home_goals), float(pred_away_goals),
...
def keep_only_current_fixtures(predictions_df, fixtures_df):
    ...
    # Keep only fixtures that still belong to the latest cup slate.
    frame = predictions_df.copy()
...
def parse_cli_args():
    parser = argparse.ArgumentParser(
        description="Predict next cup fixtures and track outcomes for later comparison."
    )
    parser.add_argument(
        "--refresh-download",
        action="store_true",
    ...
    parser.add_argument(
        "--window-days",
        type=int,
        default=DEFAULT_CUP_FIXTURE_WINDOW_DAYS,
        help="Fallback days after a cup's first upcoming fixture for cups without a configured fixture window.",
    )
    parser.add_argument(
        "--api-token",
        type=str,
...
def calculate_fixture_window_end(window_days, start_date=None):
    # Anchor to the supplied start date, usually the cup's first upcoming fixture date.
...
def filter_to_next_fixture_window(fixtures, window_days, label):
    ...
    today = pd.Timestamp(datetime.now(UTC).date()).normalize()
    _, cutoff_date = season_calendar.cup_lookahead_bounds(
        today,
        lookahead_days=window_days,
    )
    filtered = frame[
        (frame["match_date"] >= today) & (frame["match_date"] <= cutoff_date)
    ].reset_index(drop=True)
    print(
        f"{label}: Cup rolling window {today.date()} through {cutoff_date.date()} "
        f"({window_days} days, {len(filtered)} fixtures)."
    )
    return filtered
...
def normalize_team_key(name):
    ...
    stop_words = {
        "fc", "cf", "ac", "ca", "afc", "us", "sc", "sv", "fk", "the", "club", "de", "calcio", "team", "football", "sociedad", "city", "united", "town", "athletic", "county", "albion", "wanderers", "hotspur",
    }
...
def resolve_leagues_cup_team(raw_name, mapping, context):
    """Resolve a Leagues Cup team via MLS or Liga MX mappings."""
    ...
    for source_comp in LEAGUES_CUP_SOURCE_COMPETITIONS:
        mapped = str(mapping.get(source_comp, {}).get(api_name, "")).strip()
        if mapped:
            team_competition_map[mapped] = source_comp
            return mapped
        resolved = resolve_live_team_name(api_name, source_comp, context)
        if resolved:
            team_competition_map[resolved] = source_comp
            return resolved
    return None
...
def _load_name_mapping_flat():
    ...
    try:
        ...
        flat = {}
        # Prefer longer / more specific canons when the same raw key appears
        # in multiple competitions (e.g. "Inter" Serie A vs Leagues Cup).
        for comp, entries in data.items():
            ...
            for k, v in entries.items():
                ...
                if not key:
                    continue
                # Leagues Cup / MLS Inter Miami aliases win over Serie A Inter.
                if key in {"inter", "miami", "inter miami", "inter miami cf"} and str(comp).startswith(
                ...
                if key not in flat:
                    flat[key] = v
                    continue
                # Keep the longer canonical when colliding.
                if len(str(v)) > len(str(flat[key])):
                    flat[key] = v
    ...
    for _, row in fixtures.iterrows():
        ...
        for side_col in ["home_team", "away_team"]:
            ...
            if not existing:
                ...
                target = resolved if resolved else (api_name if api_name in canonical_names else "")
...
def apply_team_mapping_to_fixtures(fixtures, mapping, context):
    ...
    def mapped_name(competition, api_name):
        ...
        if api_name in known_teams:
            return api_name
        if api_name in context.get("preseason_known_teams", set()):
            return api_name
        if competition in UEFA_CUP_COMPETITIONS:
            resolved = resolve_live_team_name(api_name, competition, context, mapping=mapping)
            if resolved:
                return resolved
            return api_name
        if competition in CROSS_LEAGUE_CUP_COMPETITIONS:
            resolved = resolve_leagues_cup_team(api_name, mapping, context)
            if resolved:
                return resolved
            return api_name
        if competition in AUTO_RESOLVE_CUP_COMPETITIONS:
            resolved = resolve_live_team_name(api_name, competition, context, mapping=mapping)
            if resolved:
                return resolved
        resolved = resolve_live_team_name(api_name, competition, context, mapping=mapping)
...
def _enrich_uefa_context_for_known_team(team_name, competition, season_key, context):
    """Attach UEFA domestic league + coefficient data for teams already in the DB.

    Cup predictions for known teams still use their trained overall/season/form
    rows, but should also use country/league coefficients and (when available)
    domestic table position — the same signals used for missing-team fallbacks.
    """
    if not str(competition or "").startswith("Europe/"):
        return
    if "uefa_coefficients" not in context:
        return
    ...
    if not uefa_data or not uefa_data.get("league"):
        return
    ...
    context.setdefault("team_competition_map", {})[team_name] = real_league
    if uefa_data.get("squad_value_eur_m") is not None:
        context.setdefault("uefa_squad_values", {})[team_name] = uefa_data["squad_value_eur_m"]
    domestic = uefa_data.get("domestic") or {}
    if not domestic:
        return
    season_teams = context.setdefault("season_teams", {})
    season_bucket = season_teams.setdefault(season_key, {})
    existing = season_bucket.get(team_name) or {}
    played = max(1, int(domestic.get("played", existing.get("games", 1) or 1)))
    points = float(domestic.get("points", existing.get("points", 0) or 0))
    # Prefer live domestic table numbers when the local season row is empty/thin.
    if int(existing.get("games", 0) or 0) < 3:
        season_bucket[team_name] = {
            **existing,
            "games": played,
            "points": points,
            "avg_goals_scored": float(existing.get("avg_goals_scored", domestic.get("goals_for", 0) / played)),
            "avg_goals_conceded": float(existing.get("avg_goals_conceded", domestic.get("goals_against", 0) / played)),
        }
...
def inject_fallback_team(team_name, competition, season_key, context):
    ...
    if team_name in overall_teams:
        team_competition_map.setdefault(team_name, competition)
        ...
        # Known teams still get UEFA domestic-league mapping + coefficient /
        # table-position enrichment for Champions/Europa/Conference fixtures.
        _enrich_uefa_context_for_known_team(team_name, competition, season_key, context)
        return

    if competition in CROSS_LEAGUE_CUP_COMPETITIONS:
        comp_teams = [
            (t, c)
            for t, c in team_competition_map.items()
            if c in LEAGUES_CUP_SOURCE_COMPETITIONS
        ]
        if not comp_teams:
            for src in LEAGUES_CUP_SOURCE_COMPETITIONS:
                src_teams = [
                    t for t, c in team_competition_map.items()
                    if c == src or c == competition
                ]
                if src_teams:
                    comp_teams = [(t, src) for t in src_teams]
        league_strength = context.setdefault("league_strength", {})
        for src_comp, src_val in CROSS_LEAGUE_LEAGUE_STRENGTHS.items():
            league_strength.setdefault(src_comp, src_val)
        # Determine which source league this team belongs to
        team_src = team_competition_map.get(team_name)
        if team_src not in LEAGUES_CUP_SOURCE_COMPETITIONS:
            team_src = None
            for src in LEAGUES_CUP_SOURCE_COMPETITIONS:
                src_teams = [t for t, _ in comp_teams if _ == src]
                if team_name in src_teams:
                    team_src = src
                    break
        # Collect rows for the team's own league (preferred) or both leagues combined
        own_rows = [overall_teams[t] for t, _ in comp_teams if _ == team_src and t in overall_teams] if team_src else []
        fallback_rows = [overall_teams[t] for t, _ in comp_teams if t in overall_teams]
        rows = own_rows if own_rows else fallback_rows
        if rows:
            # Cross-league weighting: a club from a stronger league (MLS vs
            # Liga MX) or with a richer squad should not be forced onto an
            # identical league-average row.  Scale by (league_strength / 0.85)
            # and the squad-value factor, then let the per-match randomizer
            # add an uncertainty band in predict_fixture.
            src_strength = float(league_strength.get(team_src, 0.75)) if team_src else 0.75
            squad_val = context.get("_mls_squad_values", {}).get(team_name)
            value_scale = uefa.squad_value_scale_factor(squad_val)
            scale = max(0.6, min(1.2, src_strength / 0.85)) * value_scale
            overall_teams[team_name] = {
                "games": max(1, int(round(mean_from_dicts(rows, "games", 30)))),
                "goals_scored": mean_from_dicts(rows, "goals_scored", 40.0) * scale,
                "goals_conceded": mean_from_dicts(rows, "goals_conceded", 40.0) * (2.0 - scale),
                "home_games": max(1, int(round(mean_from_dicts(rows, "home_games", 15)))),
                "away_games": max(1, int(round(mean_from_dicts(rows, "away_games", 15)))),
                "home_goals_scored": mean_from_dicts(rows, "home_goals_scored", 20.0) * scale,
                "away_goals_scored": mean_from_dicts(rows, "away_goals_scored", 20.0) * scale,
                "avg_goals_scored": mean_from_dicts(rows, "avg_goals_scored", 1.35) * scale,
                "avg_goals_conceded": mean_from_dicts(rows, "avg_goals_conceded", 1.35) * (2.0 - scale),
                "avg_home_goals_scored": mean_from_dicts(rows, "avg_home_goals_scored", 1.45) * scale,
                "avg_home_goals_conceded": mean_from_dicts(rows, "avg_home_goals_conceded", 1.20) * (2.0 - scale),
                "avg_away_goals_scored": mean_from_dicts(rows, "avg_away_goals_scored", 1.20) * scale,
                "avg_away_goals_conceded": mean_from_dicts(rows, "avg_away_goals_conceded", 1.45) * (2.0 - scale),
                "avg_home_shots_for": mean_from_dicts(rows, "avg_home_shots_for", 12.0) * scale,
                "avg_home_shots_against": mean_from_dicts(rows, "avg_home_shots_against", 12.0) * (2.0 - scale),
                "avg_away_shots_for": mean_from_dicts(rows, "avg_away_shots_for", 10.5) * scale,
                "avg_away_shots_against": mean_from_dicts(rows, "avg_away_shots_against", 12.5) * (2.0 - scale),
                "weighted_avg_goals_scored": mean_from_dicts(rows, "weighted_avg_goals_scored", 1.35) * scale,
            }
            own_season = [s for src in ([team_src] if team_src else []) for s in list(season_teams.get(season_key, {}).values())]
            fallback_season = [s for src in LEAGUES_CUP_SOURCE_COMPETITIONS for s in list(season_teams.get(season_key, {}).values())]
            season_rows_list = own_season if own_season else fallback_season
            season_teams.setdefault(season_key, {})
            season_teams[season_key][team_name] = {
                "games": max(1, int(round(mean_from_dicts(season_rows_list, "games", 20)))),
                "points": mean_from_dicts(season_rows_list, "points", 28.0) * scale,
                "avg_goals_scored": mean_from_dicts(rows, "avg_goals_scored", 1.35) * scale,
                "avg_goals_conceded": mean_from_dicts(rows, "avg_goals_conceded", 1.35) * (2.0 - scale),
                "avg_home_goals_scored": mean_from_dicts(rows, "avg_home_goals_scored", 1.45) * scale,
                "avg_home_goals_conceded": mean_from_dicts(rows, "avg_home_goals_conceded", 1.20) * (2.0 - scale),
            ...
            current_form["teams"].setdefault(
                team_name,
                {
                    "points_last_10": (12.0 if team_src != "Mexico/Liga MX" else 11.0) * scale,
                    "wins_last_10": int(round((4 if team_src != "Mexico/Liga MX" else 3) * scale)),
                    "losses_last_10": int(round((3 if team_src != "Mexico/Liga MX" else 4) * (2.0 - scale))),
                    "avg_goals_for_last_10": (1.3 if team_src != "Mexico/Liga MX" else 1.2) * scale,
                    "avg_goals_against_last_10": (1.3 if team_src != "Mexico/Liga MX" else 1.4) * (2.0 - scale),
                    "previous_match_win_odds": 2.8 if team_src != "Mexico/Liga MX" else 3.1,
                    "previous_match_draw_odds": 3.3,
                    "previous_match_lose_odds": 2.8 if team_src != "Mexico/Liga MX" else 2.5,
                },
            )
            team_competition_map[team_name] = team_src if team_src in LEAGUES_CUP_SOURCE_COMPETITIONS else competition
            if team_name not in available_teams:
                available_teams.append(team_name)
            return
    ...
    if has_uefa:
        ...
        # Combine the league coefficient with the team's own squad market
        # value: a side worth noticeably more/less than a typical squad in
        # a league of this strength nudges the synthetic attack/defense
        # estimate accordingly (used only for teams missing from the
        # training database — see the `if team_name in overall_teams` guard
        # above).
        ...
        preseason_known = context.get("preseason_known_teams", set())
        is_preseason_only = team_name in preseason_known
        # Newly promoted / relegation-battling teams get a weakness factor:
        # fewer goals scored, more conceded, fewer points.
        prom = 0.85 if is_preseason_only else 1.0
        ...
        overall_teams[team_name] = {
            ...
            "goals_scored": mean_from_dicts(comp_overall_rows, "goals_scored", 40.0) * prom,
            "goals_conceded": mean_from_dicts(comp_overall_rows, "goals_conceded", 40.0) / prom,
            ...
            "home_goals_scored": mean_from_dicts(comp_overall_rows, "home_goals_scored", 20.0) * prom,
            "away_goals_scored": mean_from_dicts(comp_overall_rows, "away_goals_scored", 20.0) * prom,
            "avg_goals_scored": mean_from_dicts(comp_overall_rows, "avg_goals_scored", 1.35) * prom,
            "avg_goals_conceded": mean_from_dicts(comp_overall_rows, "avg_goals_conceded", 1.35) / prom,
            "avg_home_goals_scored": mean_from_dicts(comp_overall_rows, "avg_home_goals_scored", 1.45) * prom,
            "avg_home_goals_conceded": mean_from_dicts(comp_overall_rows, "avg_home_goals_conceded", 1.20) / prom,
            "avg_away_goals_scored": mean_from_dicts(comp_overall_rows, "avg_away_goals_scored", 1.20) * prom,
            "avg_away_goals_conceded": mean_from_dicts(comp_overall_rows, "avg_away_goals_conceded", 1.45) / prom,
            "avg_home_shots_for": mean_from_dicts(comp_overall_rows, "avg_home_shots_for", 12.0) * prom,
            "avg_home_shots_against": mean_from_dicts(comp_overall_rows, "avg_home_shots_against", 12.0) / prom,
            "avg_away_shots_for": mean_from_dicts(comp_overall_rows, "avg_away_shots_for", 10.5) * prom,
            "avg_away_shots_against": mean_from_dicts(comp_overall_rows, "avg_away_shots_against", 12.5) / prom,
            "weighted_avg_goals_scored": mean_from_dicts(comp_overall_rows, "weighted_avg_goals_scored", 1.35) * prom,
        }
        prom_season = (0.75 if is_preseason_only else 1.0)
        season_teams.setdefault(season_key, {})
        season_teams[season_key][team_name] = {
            "games": max(1, int(round(mean_from_dicts(season_rows, "games", 20)))),
            "points": mean_from_dicts(season_rows, "points", 28.0) * prom_season,
            "avg_goals_scored": mean_from_dicts(season_rows, "avg_goals_scored", 1.30) * prom,
            "avg_goals_conceded": mean_from_dicts(season_rows, "avg_goals_conceded", 1.30) / prom,
            "avg_home_goals_scored": mean_from_dicts(season_rows, "avg_home_goals_scored", 1.45) * prom,
            "avg_home_goals_conceded": mean_from_dicts(season_rows, "avg_home_goals_conceded", 1.20) / prom,
            "avg_away_goals_scored": mean_from_dicts(season_rows, "avg_away_goals_scored", 1.15) * prom,
            "avg_away_goals_conceded": mean_from_dicts(season_rows, "avg_away_goals_conceded", 1.50) / prom,
            "avg_home_shots_for": mean_from_dicts(season_rows, "avg_home_shots_for", 12.0) * prom,
            "avg_home_shots_against": mean_from_dicts(season_rows, "avg_home_shots_against", 12.0) / prom,
            "avg_away_shots_for": mean_from_dicts(season_rows, "avg_away_shots_for", 10.5) * prom,
            "avg_away_shots_against": mean_from_dicts(season_rows, "avg_away_shots_against", 12.5) / prom,
        }
        current_form.setdefault("teams", {})
        form_prom = 0.80 if is_preseason_only else 1.0
        current_form["teams"].setdefault(
            team_name,
            {
                "points_last_10": 12.0 * form_prom,
                "wins_last_10": 3.0 * form_prom,
                "losses_last_10": max(1, 10 - int(round(3.0 * form_prom)) - 3),
                "avg_goals_for_last_10": 1.2 * prom,
                "avg_goals_against_last_10": 1.2 / prom,
                "previous_match_win_odds": 2.8 / form_prom,
                "previous_match_draw_odds": 3.3,
                "previous_match_lose_odds": 2.8 / form_prom,
            },
        )

    fallback_league = (
        context.get("_uefa_team_league", {}).get(team_name)
        or context.get("_preseason_team_league", {}).get(team_name)
    )
    team_competition_map[team_name] = fallback_league or competition
    if team_name not in available_teams:
        available_teams.append(team_name)
...
def load_preseason_json(path):
    ...
    try:
        ...
        print(f"Preseason fallback: could not read {path}: {exc}")
        return {}
...
def apply_preseason_league_fallback_to_context(context):
    """Inject provisional 2026-27 league context for top-5 domestic leagues.

    Only the big-5 European leagues (Premier League, La Liga, Serie A,
    Bundesliga, Ligue 1) use the preseason fallback.  Other leagues wait
    until their regular-season CSV appears and passes the month/team-count
    gates in ``_prefer_csv_path_a``.
    """
    payload = load_preseason_json(PRESEASON_LEAGUE_FALLBACK_FILE)
    leagues = payload.get("leagues", {}) if isinstance(payload, dict) else {}
    if not isinstance(leagues, dict):
        return context

    league_strength = context.setdefault("league_strength", {})
    team_competition_map = context.setdefault("team_competition_map", {})
    preseason_known = set(context.get("preseason_known_teams", set()))
    preseason_team_league = context.setdefault("_preseason_team_league", {})
    ...
    for league, info in leagues.items():
        ...
        if not league:
            continue
        if league not in TOP_FALLBACK_LEAGUES:
            continue
        try:
            strength = float(info.get("league_strength"))
        except (TypeError, ValueError):
            country = str(info.get("country", "")).strip()
            strength = uefa.get_country_strength(country) if country else None
        if strength is not None:
            league_strength.setdefault(league, strength)

        for team_name in info.get("teams", []) or []:
            team_name = str(team_name).strip()
            if not team_name:
                continue
            team_competition_map.setdefault(team_name, league)
            preseason_team_league[team_name] = league
            preseason_known.add(team_name)

    context["preseason_known_teams"] = preseason_known
    return context


def load_preseason_cup_fixtures(window_days, competition_name):
    """Return provisional preseason fixtures for API smoke tests/offseason gaps."""
    payload = load_preseason_json(PRESEASON_CUP_FIXTURES_FILE)
    ...
    for fixture in fixtures:
        if not isinstance(fixture, dict):
            continue
        if str(fixture.get("competition", "")).strip() != competition_name:
            continue
        ...
        days_from_today = fixture.get("days_from_today")
        if days_from_today is not None:
            try:
                match_date = today + pd.Timedelta(days=int(days_from_today))
            except (TypeError, ValueError):
                continue
        else:
        ...
        rows.append(
            {
                ...
                "match_datetime_et": str(fixture.get("match_datetime_et", "")).strip(),
                "competition": competition_name,
                "home_team": home_team,
                "away_team": away_team,
                "fixture_source": "preseason_fallback",
            }
        )
    ...
    if frame.empty:
        return frame
    frame = frame.sort_values(["match_date", "home_team", "away_team"]).reset_index(drop=True)
    frame = filter_to_next_fixture_window(frame, window_days, f"Preseason fallback {competition_name}")
    print(f"Preseason fallback: Loaded {len(frame)} provisional fixtures for {competition_name}.")
    return frame


def validate_cup_api_endpoints(timeout=15):
    """Probe ESPN cup scoreboard endpoints and report whether each returns data.

    The upcoming-cups loader handles empty event lists gracefully, but a 400/404
    here means the espn_id itself is wrong and should be fixed. This helper is
    intentionally side-effect-free so it can be used by smoke tests.
    """
    statuses = []
    for cup_name, cup_data in sorted(CUP_COMPETITIONS.items(), key=lambda item: item[1]["priority"]):
        espn_id = cup_data.get("espn_id")
        url = ESPN_SCOREBOARD_API.format(espn_id=espn_id)
        status = {
            "cup": cup_name,
            "competition": cup_data.get("name", ""),
            "espn_id": espn_id,
            "ok": False,
            "http_status": None,
            "events": 0,
        ...
        try:
            ...
            status["events"] = len(data.get("events") or [])
            status["ok"] = True
...
def load_upcoming_matchweek_fixtures_from_csv_fallback(window_days, competition_name):
    today = pd.Timestamp(datetime.now(UTC).date())
    source = download_latest.fetch_source_dataframe()
    needed = {"Date", "Home", "Away", "HG", "AG"}
    if not needed.issubset(source.columns):
        print(f"CSV Fallback: Missing required columns {needed}. Skipping.")
        return pd.DataFrame()
    ...
    if frame.empty:
        print("CSV Fallback: No upcoming fixtures found.")
        return pd.DataFrame()

    frame = frame.rename(columns={"Home": "home_team", "Away": "away_team"})
    frame["competition"] = competition_name # Set the correct competition name
    fixtures = frame[["match_date", "competition", "home_team", "away_team"]].copy()
    fixtures["match_datetime_et"] = None
    fixtures = fixtures.sort_values(["match_date", "home_team", "away_team"]).reset_index(drop=True)
    fixtures = filter_to_next_fixture_window(fixtures, window_days, f"CSV Fallback {competition_name}")
    print(f"CSV Fallback: Successfully filtered down to {len(fixtures)} fixtures.")
    return fixtures


def load_upcoming_matchweek_fixtures_from_espn(window_days, competition_name, espn_id, lookahead_days=365):
    today = pd.Timestamp(datetime.now(UTC).date())
    rows = []
    seen = set()
...
"""def load_upcoming_matchweek_fixtures(api_token, window_days):
    ...
    for cup_name, cup_data in sorted_cups:
        print(f"\n==================================================================")
        print(f"--- Checking Source for: {cup_name} ---")
        
        # A. Try ESPN (Highest Priority)
        fixtures = load_upcoming_matchweek_fixtures_from_espn(window_days, cup_data['name'])
        if not fixtures.empty:
            all_fixtures.append(fixtures)
            print(f"SUCCESS: Loaded {len(fixtures)} fixtures for {cup_name} from ESPN.")
            continue # Stop if successful, as ESPN is the best source
        
        # B. Try Paid API (Fallback for MLS/General Leagues)
        if cup_data['api_source'] == 'mls' and api_token:
            fixtures = load_upcoming_matchweek_fixtures_from_api(api_token, window_days)
            if not fixtures.empty:
                all_fixtures.append(fixtures)
                print(f"SUCCESS: Loaded {len(fixtures)} fixtures for {cup_name} from football-data.org API.")
                continue
        
        # C. Try CSV Fallback
        fixtures = load_upcoming_matchweek_fixtures_from_csv_fallback(window_days, cup_data['name'])
        if not fixtures.empty:
            all_fixtures.append(fixtures)
            print(f"SUCCESS: Loaded {len(fixtures)} fixtures for {cup_name} from CSV fallback.")
            continue 

    # Aggregate all fixtures into one DataFrame
    if not all_fixtures:
        print("FAILURE: Could not load any fixtures from any source for any cup.")
        return pd.DataFrame()
    
    final_fixtures = pd.concat(all_fixtures, ignore_index=True)
    print(f"\n==================================================================")
    print(f"AGGREGATION COMPLETE: Total unique fixtures loaded: {len(final_fixtures)}")
    print(f"==================================================================")
