...
# When MLS download/process has not populated this tree yet, fall back to the
# shared Europe Processed_Data so Project_League_Table can still load a model
# and write projected_league_tables.csv (avoids UNUSABLE / file-missing loops).
...
def resolve_processed_dir(preferred=None):
    """Prefer MLS Processed_Data; fall back to shared Europe when MLS is empty."""
    ...
    if preferred != SHARED_PROCESSED_DIR and processed_dir_has_season_csvs(SHARED_PROCESSED_DIR):
        print(
            f"[mls] Processed_Data empty at {preferred}; "
...
def resolve_team_data_dir(preferred=None):
    """Prefer MLS Team_Data when it has roster JSON; else shared Europe Team_Data."""
    ...
    if preferred != SHARED_TEAM_DATA_DIR and os.path.isfile(shared_marker):
        print(
            f"[mls] Team_Data missing at {preferred}; "
...
def load_training_matches(processed_dir=None):
    """Load training frames from MLS Processed_Data, with shared Europe fallback."""
    ...
    if not frames:
        raise ValueError(
            "No season CSV files found in MLS or shared Data/Processed_Data "
