...
_REGION_DIR = _os_paths_setup.path.dirname(_FILES_DIR)  # pipelines/extra
...
BASE_DIR = str(_bts_paths.EXTRA_DIR)
PREDICTIONS_DIR = str(_bts_paths.OUTPUT_PRED_EXTRA)
...
SHARED_MAPPING_FILE = str(_bts_paths.TEAM_NAME_MAPPING_MASTER)
...
def _append_mapping_if_missing(competition, unresolved_names, valid_names, roster_teams=None):
    if not unresolved_names or not os.path.exists(SHARED_MAPPING_FILE):
        return
    mapping = tmg.load_team_mapping(SHARED_MAPPING_FILE)
    ...
    if added:
        ...
        written = tmg.save_team_mapping(SHARED_MAPPING_FILE, mapping)
...
def main():
    ...
    if not comps:
        ...
        _progress(f"[league-tables] START — projecting {total} Extra competitions")
        if comp_workers <= 1 or len(comps) <= 1:
            ...
            with ProcessPoolExecutor(
                ...
                for idx, (comp, path) in enumerate(comps, start=1):
                    ...
                    futures[executor.submit(_project_competition_worker, comp, path)] = comp
