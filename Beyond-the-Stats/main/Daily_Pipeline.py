...
PREDICTIONS_DIR = _paths.OUTPUT_PRED_EUROPE
MLS_PREDICTIONS_DIR = _paths.OUTPUT_PRED_MLS
EXTRA_PREDICTIONS_DIR = _paths.OUTPUT_PRED_EXTRA
CUP_PREDICTIONS_DIR = _paths.OUTPUT_PRED_CUPS
NATIONAL_PREDICTIONS_DIR = _paths.OUTPUT_PRED_NATIONAL
FRIENDLIES_PREDICTIONS_DIR = _paths.OUTPUT_PRED_FRIENDLIES
SHARED_PREDICTIONS_DIR = _paths.OUTPUT_PRED_SHARED
...
def build_mobile_app_feed(pipeline_status, step_results, output_path):
    ...
    sources = {
        ...
        "upcoming_fixtures_cups": CUP_PREDICTIONS_DIR / "upcoming_cup_predictions.csv",
        "upcoming_fixtures_national": NATIONAL_PREDICTIONS_DIR / "upcoming_national_team_predictions.csv",
        "projected_league_tables_global": PREDICTIONS_DIR / "projected_league_tables.csv",
        "projected_cup_tables": CUP_PREDICTIONS_DIR / "projected_cup_tables.csv",
        "projected_cup_brackets": CUP_PREDICTIONS_DIR / "projected_cup_brackets.json",
        "completed_cup_predictions": CUP_PREDICTIONS_DIR / "completed_cup_predictions.csv",
        "projected_future_matches": PREDICTIONS_DIR / "projected_future_matches.csv",
        "world_cup_projection": NATIONAL_PREDICTIONS_DIR / "world_cup_projection.json",
...
def _publish_world_cup(output_dir):
    ...
    src = NATIONAL_PREDICTIONS_DIR / "world_cup_projection.json"
    if not src.exists():
        return None
...
def _publish_enriched_competition_data(output_dir, cup_brackets_path, mls_bracket_path, mls_table_path, written):
    ...
    cup_completed = _read_csv(CUP_PREDICTIONS_DIR / "completed_cup_predictions.csv")
...
def publish_to_output(output_dir=None):
    ...
    cup_tables = CUP_PREDICTIONS_DIR / "projected_cup_tables.csv"
    ...
    upcoming_sources = [
        ...
        CUP_PREDICTIONS_DIR / "upcoming_cup_predictions.csv",
    ...
    nat_sources = [NATIONAL_PREDICTIONS_DIR / "upcoming_national_team_predictions.csv"]
    friendlies_sources = [FRIENDLIES_PREDICTIONS_DIR / "upcoming_club_friendlies.csv"]
    ...
    _publish_enriched_competition_data(
        output_dir,
        CUP_PREDICTIONS_DIR / "projected_cup_brackets.json",
...
def _write_pipeline_status(results: dict) -> None:
    """Write pipeline step results to Output/Status/pipeline_status.json."""
    try:
        now = datetime.now(UTC).replace(microsecond=0)
        ...
        pfile = _paths.PIPELINE_STATUS_FILE
