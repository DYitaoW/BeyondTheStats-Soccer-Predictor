...
# main/Backend/server.py → Beyond-the-Stats/
SP_DIR = Path(__file__).resolve().parent.parent.parent
PROJECT_ROOT = SP_DIR.parent  # repo root
...
EUROPE_FILES_DIR = _paths.EUROPE_FILES_DIR
MLS_FILES_DIR = _paths.MLS_FILES_DIR
EXTRA_FILES_DIR = _paths.EXTRA_FILES_DIR
DAILY_PIPELINE = _paths.DAILY_PIPELINE
LIVE_RESULTS_UPDATER = _paths.LIVE_RESULTS_UPDATER
BACKEND_RUN_STATUS_FILE = _paths.BACKEND_RUN_STATUS_FILE
OUTPUT_LOGS_DIR = _paths.OUTPUT_LOGS_DIR
...
class BackendConfig:
    ...
    # Misc
    log_dir: Path = field(default_factory=lambda: OUTPUT_LOGS_DIR)
...
class BackendServer:
    ...
    def _pipeline_done(self, proc: subprocess.Popen) -> None:
        ...
        try:
            ...
            try:
                status_path = BACKEND_RUN_STATUS_FILE
    ...
    def _any_model_cache_missing(self) -> bool:
        ...
        if not self.config.pipeline_skip_global:
            specs.append(("global", str(EUROPE_FILES_DIR / "Predict_Match.py")))
        if not self.config.pipeline_skip_mls:
            specs.append(("mls", str(MLS_FILES_DIR / "Predict_Match.py")))
        if not self.config.pipeline_skip_extra:
            specs.append(("extra", str(EXTRA_FILES_DIR / "Predict_Match.py")))
    ...
    def _run_light_refresh(self, changed_path: Optional[Path]) -> None:
        ...
        cmd_global = [
            sys.executable,
            str(EUROPE_FILES_DIR / "Predict_Upcoming_Matchweek.py"),
            "--window-days",
            str(self.config.watcher_window_days),
        ]
        cmd_mls = [
            sys.executable,
            str(MLS_FILES_DIR / "Predict_Upcoming_Matchweek.py"),
            "--window-days",
            str(self.config.watcher_window_days),
        ]
        cmd_extra = [
            sys.executable,
            str(EXTRA_FILES_DIR / "Predict_Upcoming_Matchweek.py"),
            "--window-days",
            str(self.config.watcher_window_days),
        ]
        try:
            ...
            settle_cmd = [sys.executable, str(LIVE_RESULTS_UPDATER)]
