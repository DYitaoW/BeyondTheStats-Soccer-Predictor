...
if (typeof window !== "undefined") {
    window.BTS_API_BASE = window.BTS_API_BASE || "https://api.beyondthestatsapp.com";
    if (!window.__btsFetchWrapped) {
        window.__btsFetchWrapped = true;
        const __origFetch = window.fetch.bind(window);
        window.fetch = function (u, o) {
            if (typeof u === "string" && u.startsWith("/api/")) {
                u = String(window.BTS_API_BASE).replace(/\/$/, "") + u;
            }
...
try {
    ...
    target.textContent = "Failed to load upcoming predictions (invalid API response).";
    return;
}
if (!resp.ok || !data.ok) {
