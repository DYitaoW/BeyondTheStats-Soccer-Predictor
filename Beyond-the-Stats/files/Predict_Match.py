...
# Repo-root Data/ next to Beyond-the-Stats/ (same folder as redeem_codes.json).
...
def compute_goal_probabilities(lambda_home: float, lambda_away: float) -> dict:
    ...
    # Over goal totals: sum joint probabilities
    p_over_1_5 = 1.0
    for h in range(0, 3):
        for a in range(0, 3):
            if h + a <= 1:
...
def resolve_team_name(raw_name, valid_names):
    """Map an API/ESPN/display name onto a name present in *valid_names*.

    Uses ``team_name_mapping_master.json`` first so abbreviated football-data
    names (Man United, Man City, Nott'm Forest) resolve from ESPN ``* FC`` forms.
    """
    if not raw_name:
        return None
    ...
    for key in (lower, normalize(raw)):
    ...
    # ESPN often appends FC/AFC; strip once and retry mapping + exact match.
    stripped = re.sub(r"\s+(AFC|FC)\s*$", "", raw, flags=re.IGNORECASE).strip()
    if stripped and stripped.lower() != lower:
        for key in (stripped.lower(), normalize(stripped)):
            hit = from_candidate(mapping.get(key))
            if hit:
                return hit
